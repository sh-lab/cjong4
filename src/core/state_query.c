...
cj4_get_last_discard_tile(
    const cj4_mahjong *state)
{
    return state->discard_count == 0 ? CJ4_TILE_ID_INVALID
                                     : state->last_discard_tile;
}
...
{
    if (wall_position == CJ4_LOCATION_NONE)
...
{
    uint8_t found = 0;
    uint8_t count = state->discard_count;
...
cj4_count_melds(
    ...
    cj4_player player)
{
    return cj4_collect_melds(state, player, NULL);
}
    ...
    cj4_player player)
{
    ...
    if (masked.draw_tile != CJ4_TILE_ID_INVALID)
    {
